...
extern void bridge_png_set_quantize(png_structrp png_ptr, png_colorp palette,
                                      int num_palette, int maximum_colors,
                                      png_const_uint_16p histogram,
                                      int full_quantize);
extern void bridge_png_longjmp(png_const_structrp png_ptr, int val);
extern void bridge_png_read_row(png_structrp png_ptr, png_bytep row,
...
extern void bridge_png_process_data(png_structrp png_ptr, png_inforp info_ptr,
...
void PNGAPI png_process_data(png_structrp png_ptr, png_inforp info_ptr,
    ...
    bridge_png_process_data(png_ptr, info_ptr, buffer, buffer_size);
}
...
int png_safe_call_read_row(png_structrp png_ptr, png_bytep row, png_bytep display_row) {
    PNG_SAFE_SETJMP_BEGIN(png_ptr)
    bridge_png_read_row(png_ptr, row, display_row);
    PNG_SAFE_SETJMP_END(png_ptr)
}
...
int png_safe_call_set_quantize(png_structrp png_ptr, png_colorp palette,
                               ...
                               int full_quantize) {
    PNG_SAFE_SETJMP_BEGIN(png_ptr)
    bridge_png_set_quantize(png_ptr, palette, num_palette, maximum_colors,
